...
| **加进开始菜单**（绿色单文件没有安装器，勾一下就能在「开始」里搜到） | ✅ |
...
这是个**绿色单文件**，没有安装器，所以「开始」里默认搜不到。托盘右键或设置页勾上
「加进开始菜单」即可 —— 它在当前用户的开始菜单里放一个快捷方式，不写全局目录、不要管理员权限。
把 exe 挪走之后勾会变回未选中（快捷方式指向旧位置），重新勾一次就修好。
...
| 托盘左键 | 展开 / 收起 |
| 托盘右键 | 菜单：热键提示、频谱律动、**显示歌词**、**歌词翻译**、**降水提醒**、**Phira 成绩提醒**、**显示哪些事件**、**开机自启**、**加进开始菜单**、**设置…** |
...
├─ Core/StartMenu.cs               加进开始菜单（IShellLink 写 .lnk，只碰当前用户目录）
...
本项目**没有引入任何第三方代码**，全部实现是 C# 从零写的。但有两处参照了别的项目，
取的是**数值与行为常量**，出处逐条标在代码注释里：

| 参照 | 许可 | 取了什么 |
|---|---|---|
| [WinIsland](https://github.com/WinIslandProject/WinIsland) | GPL-3.0 | 右键工具栏的**外观数值**：内边距 5、卡片圆角 12、描边 `argb(40,255,255,255)`、图标占卡片短边 0.38、硬阴影 `argb(60,0,0,0)` 偏移 (0,+2)。动效没抄（它是纯 opacity 淡入，这里走弹簧） |
| [Phira / prpr](https://github.com/TeamFlos/phira) | GPL-3.0 | 成绩**评级阈值**（700000 / 820000 / 880000 / 920000 / 960000 / 1000000）与图标名顺序（F / C / B / A / S / V / FC / phi），取自 `prpr/src/judge.rs::icon_index` 与 `resource.rs`。自己发明一套会和游戏结算画面对不上 |

两者都是 GPL-3.0，而这里取的是"量了多少"和"游戏怎么判定"这类事实性常量，
不是它们的源码 —— 尤其评级阈值属于**为了和游戏结果一致而必须一致**的互操作常量。
运行时也不链接、不分发它们的任何部分。

Phira 联动是**单方面**的：只读它的本地存档，游戏那边不做任何改动，也不装插件。

## 许可

[MIT](LICENSE) © 2026 fttawa
