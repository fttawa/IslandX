...
绿色单文件没有安装器，「开始」里搜不到它，用户只能自己记住 exe 扔在哪。补这个缺口。

### 只写当前用户目录

快捷方式放 `%APPDATA%\Microsoft\Windows\Start Menu\Programs`，
**不碰 `%ProgramData%` 的全局目录** —— 那个要管理员权限，而这只是个托盘小程序。
和开机自启写 HKCU 是同一个取舍。

直接放在 Programs 根下，不建同名文件夹：一个文件夹里只装一个快捷方式，
在「所有应用」里反而多一层要点开。
...
- **后期绑定 `WScript.Shell`** —— 代码少得多，但依赖 Windows Script Host，
  而那在不少企业策略里是关掉的。
- **COM 的 `IShellLink`** —— 系统自带，不会被那样挡住。

选了后者。代价是要手写接口声明，而**方法的声明顺序就是 vtable 顺序**，
少一个、换一个位置，调用就会跑到相邻的槽上 —— 而且是**静默**跑偏，不报错。
用不到的方法也得占着位置。

正因为会静默失败，加了 `--startmenu` 探针做真正的往返：
写 → 读回目标 → 删 → 确认消失。光"不抛异常"证明不了写对了。

### 判据是"指向哪"，不是"存不存在"

绿色程序会被挪位置，那时旧快捷方式还在、却指向一个不存在的文件。
所以 `IsRegistered` 比的是**快捷方式的目标与当前 exe 是否同一个**，
和开机自启那边同一套判据 —— 这样"挪了位置"会显示成未勾选，重新勾一次即可修好。

### 一个会破坏被测对象的探针

`--startmenu` 第一版测完直接删掉快捷方式。但它写的是**真实的开始菜单目录**，
而用户可能已经在那里注册过了 —— 于是跑一次探针，把人家真正的快捷方式一起删了。
实测就是这么发生的：跑完自检，真实快捷方式没了。

改成先把原有的整份字节备份下来、测完原样写回（不重新生成 ——
重新生成的未必和原来那个一样，图标、窗口状态之类都在 .lnk 里）。

**会破坏被测对象的探针比没有探针更糟。**

### 顺带补的两件事

**图标。** 项目一直没有 `ApplicationIcon`，exe 用的是 .NET 通用窗口图标。
在托盘里看不出来（托盘图标是运行时自绘的），但一进开始菜单就是一个认不出的通用方块 ——
**那等于没注册**。

加了 `tools/ShotBurst` 的 makeicon 模式，生成 9 个尺寸的 .ico，产物提交进仓库
（图标是会被人看的东西，应当能在版本控制里看出它变没变，而不是每次构建重新生成一个"应该一样"的文件）。

形状沿用托盘图标那个 identity，但**反了过来**：托盘图标画在固定背景上，黑胶囊看得清；
而开始菜单／任务栏的底色深浅都有，纯黑形状在深色主题上会整个消失。
所以改成近黑的圆角"应用砖块" + 亮色胶囊。16px 下省掉摄像头小孔 —— 那个尺寸画出来只会让胶囊看着脏。

**设置页两个开关贴死了。** 加完「加进开始菜单」，它和「开机自动启动」的胶囊几乎重叠。
原因是中间那条状态提示平时是 `Collapsed`（不占位），而别处的开关后面都跟着一条**常驻**的
Hint，所以从来没暴露过这个问题。补了 12px 下边距。

### 一次操作失误

想截一张"开始菜单里搜得到"的图，用 Win 键 + `SendKeys` 自动化。开始菜单没弹出来，
而那句 "IslandX" 很可能打进了当时有焦点的窗口（实测前台是承载会话的应用）。

**不该用全局按键去自动化系统 UI** —— 它没有"点不到就失败"这种安全的失败方式，
按键总会落到某个地方去。快捷方式在不在、能不能启动、图标对不对，这三件都能直接验，
而"搜索框里能不能搜到"是 Windows Search 的索引时机问题，不是本功能的正确性问题。
